...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        ...
        if (!$kartu) {
            ...
            $stmt = $pdo->prepare("SELECT * FROM berkas WHERE user_id = ? AND jenis_berkas = 'foto' ORDER BY jenis_berkas ASC");
            $stmt->execute([$user_id]);
            $berkas = $stmt->fetch();
            if ($berkas) {
                $berkas['filename'] = basename($berkas['path_file']);
            }
    ...
    if ($action === 'generate') {
        try {
            ...
            $nomor_peserta = date("Y-")  . str_pad($user_id, 5, '0', STR_PAD_LEFT);
